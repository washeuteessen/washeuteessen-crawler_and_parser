...
class HTMLParser(object):
    ...
    def parse_html(self,skip_recipes):
        """
        ...
        # randomly open 1x unparsed raw html
        html = self.collection_raw.find_one({"parsed_status": False}, skip=skip_recipes)

        # set parsed status to false
        parsed = False

        # set skip status to false
        skip = False

        # check whether unparsed doc was found
        if html is not None:
            # create empty dict
            item = {}
            ...
            logging.info("No unparsed raw recipe found. Waiting for 5 min.")
            return 0, skip
